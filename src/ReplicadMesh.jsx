...
const syncEdgesManual = (geometry, edgesData) => {
  ...
  geometry.setAttribute("position", new THREE.Float32BufferAttribute(edgesData.lines, 3));
...
export default function ReplicadMesh({ faces, edges, mode, selectedIndices, onSelect }) {
  ...
  const getGroupIndex = (intersect) => {
    const vertexIdx = mode === "face" ? intersect.faceIndex * 3 : intersect.index;
    if (vertexIdx === undefined) return null;

    const target = mode === "face" ? bodyGeom.current : edgeGeom.current;
    return target.groups.findIndex(g => 
      vertexIdx >= g.start && vertexIdx < g.start + g.count
    );
  };

  const HighlightLayer = ({ index, color, opacity = 1, linewidth = 4, renderOrder = 5 }) => {
    ...
    return isFace ? (
      <mesh renderOrder={renderOrder}>
        ...
        <meshStandardMaterial color={color} transparent opacity={opacity} depthTest={false} />
      </mesh>
    ) : (
      <lineSegments renderOrder={renderOrder}>
        <bufferGeometry attach="geometry" {...geometryProps} />
        <lineBasicMaterial color={color} linewidth={linewidth} transparent opacity={opacity} depthTest={false} />
      </lineSegments>
    );
  };

  return (
    <group onPointerOut={() => { setHoveredIdx(null); invalidate(); }}>
      {/* 1. SOLID BODY */}
      <mesh 
        ...
        onPointerMove={(e) => {
          ...
          if (idx !== hoveredIdx) { setHoveredIdx(idx); invalidate(); }
        }}
        onClick={(e) => {
          if (mode !== "face") return;
          e.stopPropagation();
      ...
      {/* 3. HOVER HIGHLIGHT */}
      {hoveredIdx !== null && (
        <HighlightLayer index={hoveredIdx} color="white" opacity={0.8} linewidth={4} renderOrder={10} />
      )}

      {/* 4. SELECTION HIGHLIGHTS */}
      {selectedIndices[0] !== undefined && (
        <HighlightLayer index={selectedIndices[0]} color="#3b82f6" opacity={1} linewidth={6} renderOrder={20} />
      )}
      {selectedIndices[1] !== undefined && (
        <HighlightLayer index={selectedIndices[1]} color="#f472b6" opacity={1} linewidth={6} renderOrder={20} />
      )}
    </group>
  );
}